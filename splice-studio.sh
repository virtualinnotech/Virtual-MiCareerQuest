...
NEW_STUDIO="studio-frame.html"

if [ ! -f "$FILE" ]; then echo "ERROR: $FILE not found in this folder. Run this from your repo folder."; exit 1; fi
if [ ! -f "$NEW_STUDIO" ]; then echo "ERROR: $NEW_STUDIO not found in this folder."; exit 1; fi

START_TAG='<script id="studio-source" type="application/octet-stream">'
END_TAG='</script>'

TAG_START=$(grep -abo -F "$START_TAG" "$FILE" | head -1 | cut -d: -f1)
if [ -z "$TAG_START" ]; then echo "ERROR: could not find the studio-source tag in $FILE."; exit 1; fi
...
echo "Found the old studio content (bytes $CONTENT_START to $CONTENT_END). Splicing in the new version..."

TMPFILE="$FILE.tmp-new"
rm -f "$TMPFILE"
head -c "$CONTENT_START" "$FILE" > "$TMPFILE"
base64 < "$NEW_STUDIO" >> "$TMPFILE"
...
# base64's decode flag differs between GNU (-d) and BSD/macOS (-D, though some
# versions also accept -d) -- try both rather than guessing which this Mac has.
...
if [ -s "$TMPFILE.decoded" ] && cmp -s "$TMPFILE.decoded" "$NEW_STUDIO"; then
  mv "$TMPFILE" "$FILE"
